  ...
  const MeasurementItem = ({ measurement }) => {
          ...
          <View style={styles.measurementDetails}>
            <View style={styles.measurementInfo}>
              ...
              <Text style={styles.measurementDimensions}>
                {measurement.width}&quot; (
                {Number(measurement.widthFeet).toFixed(1)}ft) ×{" "}
                {measurement.height}&quot; (
                {Number(measurement.heightFeet).toFixed(1)}ft)
              </Text>
              <Text style={styles.measurementSpecs}>
                Material: {measurement.materialType || "-"} | Rate: ₹
                {measurement.materialRatePerSqft || 0}/sqft
              </Text>
              <Text style={styles.measurementSpecs}>
                Total Sqft: {Number(measurement.totalSqft).toFixed(1)} |
                Material Cost: {formatCurrency(measurement.materialCost || 0)}
              </Text>
            ...
            <Text style={styles.measurementCost}>
              {formatCurrency(measurement.materialCost || 0)}
            </Text>
          </View>
        </View>
      );
          ...
          <View style={styles.measurementDetails}>
            <View style={styles.measurementInfo}>
              ...
              >
                {measurement.parts === "One Part" ? (
                  ...
                  >
                    <Text style={{ color: "#2563eb", fontWeight: "bold" }}>
                      {Number(measurement.pieces).toFixed(1)} pieces
                    </Text>
                  </View>
                ) : (
                  <Text style={styles.measurementSpecs}>
                ...
                <Text style={styles.measurementSpecs}>
                  • {Number(measurement.totalMeters).toFixed(1)}m Cloth
                </Text>
              </View>
              <Text style={styles.measurementSpecs}>
                Cloth: {formatCurrency(measurement.clothCost || 0)} | Stitching:{" "}
                {formatCurrency(measurement.stitchingCost || 0)}
              </Text>
              {measurement.hasLining ? (
                <Text style={styles.measurementSpecs}>
                  Lining: {measurement.liningType} |{" "}
                  {Number(measurement.totalLiningMeters).toFixed(1)}m × ₹
                  {measurement.liningCostPerMeter || 0} ={" "}
                  {formatCurrency(measurement.totalLiningCost || 0)}
                </Text>
              ) : null}
            </View>
            <Text style={styles.measurementCost}>
    ...
    if (measurement.interiorType === "wallpapers") {
      ...
      const implementationCostPerRoll =
        parseFloat(measurement.implementationCostPerRoll) || 0;
        ...
        <View style={styles.measurementItem}>
          ...
          <View style={styles.measurementDetails}>
            <View style={styles.measurementInfo}>
              ...
              <Text style={styles.measurementSpecs}>
                Cost/Roll: {formatCurrency(costPerRoll)} | Impl/Roll:{" "}
                {formatCurrency(implementationCostPerRoll)}
              </Text>
              <Text style={styles.measurementSpecs}>
                Material: {formatCurrency(totalMaterialCost)} | Install:{" "}
  ...
  return (
    <View style={styles.container}>
      ...
      {measurements.length === 0 ? (
        <Text style={styles.emptyMessage}>No measurements added yet</Text>
      ) : (
