...
func ChangeAdminPassword(c *gin.Context) {
	...
	err := db.Collection("admins").FindOne(ctx, bson.M{"_id": adminId}).Decode(&admin)
	if err != nil {
	...
	_, err = db.Collection("admins").UpdateOne(ctx, bson.M{"_id": adminId}, bson.M{"$set": bson.M{"passwordHash": string(hash)}})
	if err != nil {
