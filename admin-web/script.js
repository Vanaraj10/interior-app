...
const API_BASE_URL = "https://interior-app-production-3afe.up.railway.app/api";
...
async function loadDashboardData() {
  ...
  try {
    Promise.all([loadWorkers(), loadProjects()]).then(() => {
...
function renderProjectsTable() {
  ...
  if (projects.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="7" style="text-align: center; padding: 2rem; color: var(--text-muted);">No projects found</td></tr>';
    return;
  }
...
function renderWorkersGrid() {
  ...
  if (workers.length === 0) {
    container.innerHTML =
      '<p style="text-align: center; color: var(--text-muted); padding: 2rem; grid-column: 1/-1;">No workers found</p>';
    return;
  }
  ...
  container.innerHTML = workers
    .map((worker) => {
      ...
      const completedProjects = workerProjects.filter(
        (p) => p.isCompleted,
      ).length;
...
async function viewProject(projectId) {
  try {
    ...
    console.log(
      ...
      project.html ? project.html.substring(0, 200) : "No HTML",
    );
    ...
    console.log("Stitching button debug:", {
      ...
      rawDataPreview: project.rawData
        ? project.rawData.substring(0, 200)
        : "No rawData",
    });
...
async function toggleProjectStatus(projectId, newStatus) {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/admin/projects/${projectId}/completed`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${authToken}`,
    ...
    if (response.ok) {
      showToast(
        `Project marked as ${newStatus ? "completed" : "pending"}`,
        "success",
      );
      await loadProjects();
      updateDashboardStats();
...
async function deleteProject(projectId, clientName) {
  if (
    !confirm(
      `Are you sure you want to delete the project for "${clientName}"? This action cannot be undone.`,
    )
  ) {
    return;
  }
...
async function deleteWorker(workerId, workerName) {
  if (
    !confirm(
      `Are you sure you want to delete worker "${workerName}"? This action cannot be undone.`,
    )
  ) {
    return;
  }
...
function checkIfProjectHasCurtains(project) {
  try {
    ...
    const hasCurtains = measurements.some(
      (measurement) => measurement.interiorType === "curtains",
    );
...
async function generateStitchingQuotation() {
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/admin/projects/${currentProject.id}/stitching-quotation`,
      {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.error || "Failed to generate stitching quotation",
      );
    }
    ...
    displayStitchingQuotation(
      data.html,
      `Stitching Unit Quotation - ${data.clientName}`,
    );
    ...
    showToast(
      error.message || "Failed to generate stitching quotation",
      "error",
    );
  } finally {
    showLoading(false);
...
function displayStitchingQuotation(
  stitchingHTML,
  title = "Stitching Unit Quotation",
) {
  ...
  iframe.onload = () => {
    try {
      ...
      iframeDoc.close();
      console.log(
        "Stitching quotation HTML content written to iframe successfully",
      );
    } catch (error) {
